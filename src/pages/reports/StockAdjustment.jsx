  ...
  return (
    <div>
      <div className="page-hero" style={{ margin: "-26px -28px 20px -28px", background: "#201e1d", padding: "20px 24px 0", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ marginBottom: 16, position: "relative", zIndex: 1 }}><div style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 10, fontWeight: 700, letterSpacing: "1.4px", textTransform: "uppercase", color: "#e15b47", marginBottom: 6 }}><div style={{ width: 5, height: 5, borderRadius: "50%", background: "#dd2b0f", animation: "pulse 2.4s ease-in-out infinite" }} />Inventory</div><div style={{ fontSize: 22, fontWeight: 900, color: "#fff", letterSpacing: "-1.2px", marginBottom: 3 }}>Stock <span style={{ background: "linear-gradient(135deg,#ff6a4d,#dd2b0f)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Adjustment</span></div><div style={{ fontSize: 12, color: "rgba(255,255,255,.4)" }}>Quickly update stock levels from anywhere</div></div>
        <div className="kpi-strip" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderTop: "1px solid rgba(255,255,255,.08)" }}>
          {[{label:"Products",val:products.length,sub:"in catalogue",accent:"#dd2b0f"},{label:"Low Stock",val:products.filter(p=>p.stock_qty<=(p.reorder_level||5)).length,sub:"need restocking",accent:products.filter(p=>p.stock_qty<=(p.reorder_level||5)).length>0?"#dc2626":"#16a34a"},{label:"Stock Value",val:fmt(products.reduce((s,p)=>s+p.stock_qty*p.cost_price,0)),sub:"at cost price",accent:"#57534e"},{label:"Retail Value",val:fmt(products.reduce((s,p)=>s+p.stock_qty*p.sale_price,0)),sub:"at sale price",accent:"#16a34a"}].map((k,i)=>(
