  ...
  return (
    <div>
      <div className="page-hero" style={{ margin: "-26px -28px 20px -28px", background: "#201e1d", padding: "20px 24px 0", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ marginBottom: 16, position: "relative", zIndex: 1 }}><div style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 10, fontWeight: 700, letterSpacing: "1.4px", textTransform: "uppercase", color: "#e15b47", marginBottom: 6 }}><div style={{ width: 5, height: 5, borderRadius: "50%", background: "#dd2b0f", animation: "pulse 2.4s ease-in-out infinite" }} />Finance</div><div style={{ fontSize: 22, fontWeight: 900, color: "#fff", letterSpacing: "-1.2px", marginBottom: 3 }}>Financial <span style={{ background: "linear-gradient(135deg,#ff6a4d,#dd2b0f)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Reports</span></div><div style={{ fontSize: 12, color: "rgba(255,255,255,.4)" }}>Profit & Loss and Balance Sheet</div></div>
        <div className="kpi-strip" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderTop: "1px solid rgba(255,255,255,.08)" }}>
          {[{label:"Total Income",val:fmt(totalRev),sub:"revenue accounts",accent:"#16a34a"},{label:"Total Expenses",val:fmt(totalExp),sub:"expense accounts",accent:"#dc2626"},{label:"Net Profit",val:fmt(Math.abs(net)),sub:net>=0?"profit":"loss",accent:net>=0?"#16a34a":"#dc2626"},{label:"Accounts",val:accounts.length,sub:"chart of accounts",accent:"#dd2b0f"}].map((k,i)=>(
