...
<div class="footer-box"><div><b>${COMPANY.name}</b> · ${COMPANY.address}, ${COMPANY.address2}, ${COMPANY.city}, ${COMPANY.county}, ${COMPANY.postcode}</div><div>VAT: ${COMPANY.vatNumber} · Tel: ${COMPANY.phone} · ${COMPANY.email}</div></div>
  ...
  return (
    <div>
      ...
      {showForm && <ModalPortal><div style={{position:"fixed",inset:0,zIndex:600,background:"var(--bg)",overflowY:"auto"}}><InvoiceForm contacts={contacts} setContacts={setContacts} products={products} accounts={accounts} token={token} userId={userId} invoices={invoices} onSave={inv => { setInvoices(prev => { if (prev.find(i=>i.id===inv.id)) return prev; return [inv,...prev]; }); setTimeout(() => sb.get(token,"invoices","order=created_at.desc&limit=1000").then(d=>Array.isArray(d)&&setInvoices(d)), 1000); }} onClose={() => setShowForm(false)} /></div></ModalPortal>}
      <div className="card">
