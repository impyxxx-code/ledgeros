...
export function InvoiceForm({ contacts, setContacts, products, accounts = [], token, userId, onSave, onClose, invoices = [], initialCustomer = "" }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const effStatus = f.status;   // receipts carry normal statuses too — paid is not forced
      const invoice_number = await nextDocNumber(token, { prefix: isReceipt ? "RCT" : "INV", table: "invoices", column: "invoice_number", width: 4 });
      ...
      if (inv && inv[0]) {
        let created = inv[0];
        logAudit(token, userId, "invoice_created", "invoice", created.id, `${isReceipt ? "Receipt" : "Invoice"} ${invoice_number} created for ${f.customer} — ${new Intl.NumberFormat("en-GB",{style:"currency",currency:"GBP"}).format(total)}${creditBlocked && creditOverride ? ` ⚠️ CREDIT OVERRIDE (${onHold ? "on hold" : "over limit"})` : ""}`);
        if (effStatus !== "draft") postInvoiceJournal(token, accounts, { invoice_id: created.id, invoice_number, amount: total, date: f.invoice_date });
        ...
        if (effStatus === "paid") {
          const res = await settleInvoice({ token, invoice: created, payNow: total, method: isReceipt ? rctMethod : "cash", date: f.invoice_date, accounts, userId });
          if (res.ok) {
            created = { ...created, amount_paid: res.amountPaid, balance: res.balance, status: res.status, payment_method: isReceipt ? rctMethod : "cash" };
  ...
  const printInvoice = async () => {
    ...
    const isRct = String(savedInvoice.invoice_number || '').startsWith('RCT-');
    const paidAmt = savedInvoice.status === 'paid' ? (sub + vat) : parseFloat(savedInvoice.amount_paid || 0);
...
.bta{position:fixed;top:0;left:0;right:0;z-index:9999;background:#201e1d;padding:14px 16px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 4px 12px rgba(0,0,0,.4)}.bta a{width:36px;height:36px;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.3);border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:22px;font-weight:300;text-decoration:none;cursor:pointer;flex-shrink:0;line-height:1}.bta a:hover{background:rgba(255,255,255,.22)}.bta-l{font-size:13px;color:rgba(255,255,255,.5);margin-top:2px}.bta-t{font-size:16px;font-weight:700;color:#fff}body{padding-top:calc(30mm + 54px)}@page{margin:0}@media print{.bta{display:none!important}body{padding:12mm 14mm!important}}</style>
</head>
    ...
    return (
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div style={{ background: "linear-gradient(135deg,#10b981,#059669)", padding: "20px 24px", display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 17, fontWeight: 700, color: "#fff", marginBottom: 2 }}>{String(savedInvoice?.invoice_number || "").startsWith("RCT") ? "Receipt" : "Invoice"} Created Successfully!</div>
        ...
        {!dnSaved && (
          <div style={{ padding: "24px 24px 20px" }}>
            ...
            <button onClick={onClose} style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, width:"100%", background:"#201e1d", border:"none", borderRadius:12, padding:"18px 20px", cursor:"pointer", fontFamily:"var(--sans)", marginBottom:16 }}>
    ...
    <div style={{ display:"flex", flexDirection:"column", minHeight:"100vh", background:"var(--bg)", paddingBottom:160 }}>
      ...
      <div style={{ background:"#201e1d", padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:50, zIndex:50 }}>
        ...
        <div style={{ textAlign:"right" }}>
          <div style={{ fontSize:18, fontWeight:800, color:"#ff6a4d", fontFamily:"var(--mono)" }}>{fmt(total)}</div>
      ...
      <details style={{ margin:"8px 12px 0", background:"var(--white)", borderRadius:"var(--rl)", border:"1px solid var(--border)", overflow:"hidden" }}>
        ...
        <div style={{ padding:"0 16px 16px", borderTop:"1px solid var(--border)" }}>
          <div style={{ marginTop:12 }}><label style={{ fontSize:11, fontWeight:600, color:"var(--text3)", display:"block", marginBottom:4 }}>Document</label><div style={{ display:"flex", gap:6 }}>{["invoice","receipt"].map(t => <button key={t} type="button" onClick={() => setDocType(t)} style={{ flex:1, padding:"11px 10px", borderRadius:8, border: docType===t ? "2px solid #dd2b0f" : "1px solid var(--border2)", background: docType===t ? "#fff5f3" : "var(--white)", color: docType===t ? "#dd2b0f" : "var(--text2)", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"var(--sans)", textTransform:"capitalize" }}>{t}</button>)}</div></div>
          ...
          {docType === "receipt" && f.status === "paid" && <div style={{ marginTop:12 }}><label style={{ fontSize:11, fontWeight:600, color:"var(--text3)", display:"block", marginBottom:4 }}>Payment Method</label><select className="il-input" value={rctMethod} onChange={e => setRctMethod(e.target.value)}><option value="cash">Cash</option><option value="card">Card</option><option value="bank">Bank Transfer</option></select></div>}
  ...
  return (
    <div className="card">
      ...
      <div className="fg">
        ...
        <div className="fgrp"><label>Document</label><div style={{ display: "flex", gap: 6 }}>{["invoice", "receipt"].map(t => <button key={t} type="button" onClick={() => setDocType(t)} style={{ flex: 1, padding: "9px 10px", borderRadius: 8, border: docType === t ? "2px solid #dd2b0f" : "1px solid var(--border2)", background: docType === t ? "#fff5f3" : "var(--white)", color: docType === t ? "#dd2b0f" : "var(--text2)", fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "var(--sans)", textTransform: "capitalize" }}>{t}</button>)}</div></div>
        ...
        {docType === "receipt" && f.status === "paid" && <div className="fgrp"><label>Payment Method</label><select value={rctMethod} onChange={e => setRctMethod(e.target.value)}><option value="cash">Cash</option><option value="card">Card</option><option value="bank">Bank Transfer</option></select></div>}
