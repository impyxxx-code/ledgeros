...
export function InvoiceForm({ contacts, setContacts, products, accounts = [], token, userId, onSave, onClose, invoices = [], initialCustomer = "" }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const effStatus = isReceipt ? "paid" : f.status;   // a receipt is always a settled sale
  ...
  return (
    <div className="card">
      ...
      <div className="fg">
        ...
        {docType === "receipt"
          ? <div className="fgrp"><label>Payment Method</label><select value={rctMethod} onChange={e => setRctMethod(e.target.value)}><option value="cash">Cash</option><option value="card">Card</option><option value="bank">Bank Transfer</option></select></div>
