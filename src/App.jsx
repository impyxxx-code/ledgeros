...
function InvoiceModal({ invoice, onClose, contacts = [], onStatusChange, onDuplicate, onEdit, onPartPay, onLogPartPay, token, profile }) {
  ...
  const handlePrint = async () => {
    ...
    if (!win) return;
    win.document.write(`<!DOCTYPE html><html><head><title>Loading...</title></head><body style="font-family:sans-serif;background:#0d1829;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh;margin:0"><div style="text-align:center"><div style="font-size:22px;font-weight:700;margin-bottom:8px">Preparing invoice…</div><div style="color:rgba(255,255,255,.5);font-size:14px">Fetching outstanding balance</div></div></body></html>`);
...
.back-to-app{position:fixed;top:0;left:0;right:0;z-index:9999;background:#0d1829;padding:14px 16px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 4px 12px rgba(0,0,0,.4)}.back-to-app a,.back-to-app button{background:rgba(99,102,241,.15);border:1px solid rgba(99,102,241,.4);border-radius:10px;padding:10px 18px;color:#c7d2fe;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:8px;transition:background .15s;text-decoration:none}.back-to-app a:hover,.back-to-app button:hover{background:rgba(99,102,241,.28)}.back-to-app-label{font-size:13px;color:rgba(255,255,255,.5);margin-top:2px}.back-to-app-title{font-size:16px;font-weight:700;color:#fff}body{padding-top:68px}@media print{.back-to-app{display:none!important}body{padding-top:0!important}}</style>
</head>
<body><div class="back-to-app"><div><div class="back-to-app-title">LedgerOS</div><div class="back-to-app-label">Arkham Retail Ltd</div></div><button onclick="window.close();"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg> Back to LedgerOS</button></div>
<div class="page">
  ...
  const downloadDNpdf = (html, dn_num) => {
    const mobWin = window.open('', '_blank', 'width=1100,height=800,scrollbars=yes,resizable=yes');
    if (!mobWin) return;
