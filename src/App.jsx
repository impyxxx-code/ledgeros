...
<div class="header">
  <div><img src="${LOGO}" style="height:52px;object-fit:contain;display:block" alt="${COMPANY.name}"/><div class="co-detail" style="margin-top:8px">${COMPANY.address}<br>${COMPANY.city}, ${COMPANY.postcode}<br>Tel: ${COMPANY.phone} · ${COMPANY.email}<br>VAT: ${COMPANY.vatNumber}</div></div>
...
export default function App() {
  ...
  const InstallBanner = showInstallBanner && isMobile() ? (
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 9999, padding: "12px 16px 20px", background: "#060d1f", borderTop: "1px solid rgba(255,255,255,.1)", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 -4px 24px rgba(0,0,0,.4)" }}>
      ...
      <button onClick={handleInstall} style={{ background: "linear-gradient(135deg,#2563eb,#1d4ed8)", color: "#fff", border: "none", borderRadius: 9, padding: "9px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "var(--sans)", whiteSpace: "nowrap", boxShadow: "0 2px 8px rgba(37,99,235,.4)", flexShrink: 0 }}>
        Add
      </button>
      <button onClick={() => setShowInstallBanner(false)} style={{ background: "none", border: "none", color: "rgba(255,255,255,.35)", cursor: "pointer", padding: 4, display: "flex", alignItems: "center", flexShrink: 0 }}>
