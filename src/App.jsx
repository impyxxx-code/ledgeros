...
function InvoiceModal({ invoice, onClose, contacts = [], onStatusChange, onDuplicate, onEdit, onPartPay, onLogPartPay, token, profile }) {
  ...
  const handlePrint = async () => {
    ...
    const _b=new Blob([html],{type:'text/html'});const _u=URL.createObjectURL(_b);const _f=Object.assign(document.createElement('iframe'),{src:_u,style:'position:fixed;width:0;height:0;border:none;opacity:0'});document.body.appendChild(_f);_f.onload=()=>{_f.contentWindow.print();URL.revokeObjectURL(_u);setTimeout(()=>{try{document.body.removeChild(_f);}catch(e){}},3000);};
  };
