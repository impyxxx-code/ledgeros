...
function InvoiceModal({ invoice, onClose, contacts = [], onStatusChange, onDuplicate, onEdit, onPartPay, onLogPartPay, token, profile }) {
  ...
  const handlePrint = async () => {
    ...
    const _hp=html.replace('</head>','<script>window.onload=function(){setTimeout(function(){window.print();},600);}<\/script></head>');const _b=new Blob([_hp],{type:'text/html'});const _u=URL.createObjectURL(_b);window.open(_u,'_blank');setTimeout(()=>URL.revokeObjectURL(_u),15000);
  };
